...
import { Image, Input, Button, ButtonText, View, InputField, KeyboardAvoidingView } from "@gluestack-ui/themed";
...
export default function ChooseAvatar() {
    ...
    return (
        <ImageBackground source={require("../../assets/bg1.jpg")} style={styles.background} blurRadius={3}>
                ...
                <View display="flex" justifyContent="center" alignItems="center">
                    ...
                    <View style={styles.avatarContainer}>
                        {dummyAvatar.map((avatar) => (
                            <TouchableOpacity key={avatar.id} style={styles.avatarItem} onPress={() => handleAvatarPress(avatar.id)}>
                                <Image alt="avatar" source={avatar.avatar} style={styles.avatarImage} />
                            </TouchableOpacity>
                        ))}
                    </View>
                    <View style={{ marginTop: 20 }}>
                        <View style={{ flexDirection: "row", alignItems: "center" }}>
                            <Input bg="white" w={300} variant="outline" size="md">
                                <FontAwesome name="pencil-square-o" size={24} color="black" style={{ marginLeft: 10, marginTop: 10 }} />
                                <InputField placeholder="your name" />
                            </Input>
                        </View>
                    </View>
                    <View style={{ marginTop: 10 }}>
                        <Button style={{ width: 300, backgroundColor: "green" }} size="md" variant="solid" action="primary" isDisabled={false} isFocusVisible={false} onPress={() => navigation.navigate("Home" as never)}>
